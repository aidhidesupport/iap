...
「仕様を書いた」と「普及する標準になった」は別の到達点。公開やライセンスの選択はまだ行わない。事業としては、適合する案件部屋、導入・運営、接続実装、品質検証などを提供する案を比較できるが、仕様の囲い込みや本人データの蓄積を前提にしない。
...
規格本文、適合試験、実装、利用者データの取扱いは別に定める。この文書を作成した時点では、公開、ライセンスの選択、商標登録、標準化団体への提出、外部サービスへの登録は行っていない。
