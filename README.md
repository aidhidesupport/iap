...
Codex連携はIAPの一つの実装例です。このリポジトリでは仕様とガイドを扱い、試作コードの一般配布はまだ開始していません。
...
- [開発者ガイド](CODEX_GUIDE.md)：試作コードを受け取った方の導入・確認・共有。
...
正式な公開ライセンスは未確定です。[RIGHTS.md](RIGHTS.md)を確認してください。大きな改稿やコードの寄稿受付は権利関係の整理まで保留しています。Issueで議論しただけで、その人の権利がプロジェクトへ移る扱いにはしません。
