...
| 使うもの | できること |
|---|---|
| 公開Webサイト | 説明・ガイド・2分12秒の紹介動画を見る。ページを開くだけでは案件に導入されません |
| 配布物の架空例 | 端末のターミナルで、観測・共有文・古い評価の失効をひとりで確認。PMの参加やCodexへのログインは不要 |
| Codex連携 | 案件フォルダーに設定し、Codex CLIの作業中に照合を組み込む。初期設定はターミナル、その後はCodexとの会話とコマンドで作業・共有を進める |

専用GUIだけで導入から共有まで完結する実装ではありません。手入力で条件や進捗を扱うWeb先行試作もありますが、Codex連携との同期はありません。以下は配布物と、検証済みのCLIでの手順です。

まずは「入手して確かめる」の架空例まで試せます。その確認は、個人開発での継続利用や、別々のPM・開発者の負担軽減を実証するものではありません。
