...
export default function FAQ() {
  return <Guide title="試す前に、知っておきたいこと。" label="FAQ" lead="IAPはpartial-standardsの活動として提案している仕様草案です。最初の実装例であるCodex連携を中心に、役割・できること・利用条件をまとめました。" sections={[
    ...
    <Section id="start" title="01 — 誰が、どう試すか">
      <h3>どんな人に向いていますか？</h3><p>依頼の目的を取り違える、途中で範囲が広がる、進捗の説明に手間がかかる、といった協働の困りごとを想定しています。現在の試験版は、macOS/Linuxでターミナルのコマンドを実行できる開発者向けです。基本の協働場面では、PMを別の人が担当します。診断の開示は利用条件ではありません。</p>
      <h3>システムは完成していますか？</h3><p>完成条件との照合、修正・再確認、共有文生成、古い評価の失効、休止・復帰といった中核機能は実装済みで、試験版として公開しています。他の人の導入や、実際の負担軽減は未検証です。配布物0.2.5の自動テストと、同じMacでの実Codexによる作業・再評価・共有・中断復帰を確認しました。現在の0.2.6は同梱文書の修正で、実行コード・記録形式は0.2.5と同一です。</p><TextLink href="/status">実装と検証の現在地</TextLink>
